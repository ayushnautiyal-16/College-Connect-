...
import CountUpAnimation from '../../components/CountUpAnimation'; // Import the new component
...
function About() {
  ...
  const approachSteps = [
    {
      ...
      description: 'We begin by listening—understanding your interests, academic background, and career aspirations.',
    },
    {
      number: '02',
      title: 'Recommend Suitable Colleges',
      description: 'Based on your profile, we shortlist colleges that align with your goals and eligibility.',
    },
    {
      number: '03',
      title: 'Guide the Admission Process',
      description: 'We assist with applications, documentation, and all admission formalities step-by-step.',
    },
    {
      number: '04',
      title: 'Ongoing Support',
      description: "Our support continues even after admission—we're here whenever you need guidance.",
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 overflow-hidden relative">
      ...
      <section ref={heroRef.ref} className="relative z-10 text-white min-h-[90vh] md:min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className={`container mx-auto px-4 relative z-20 text-center transform transition-all duration-1000 ${heroRef.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="flex justify-center mb-6 md:mb-8">
            <span className="inline-block py-1.5 px-4 md:py-2 md:px-6 rounded-full bg-white/5 backdrop-blur-md border border-white/10 text-indigo-300 text-xs md:text-sm font-semibold tracking-wide uppercase animate-slide-up shadow-lg">
              ✨ Best Education Consultancy in Dehradun
            </span>
          </div>
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-black mb-6 md:mb-8 leading-tight tracking-tight drop-shadow-2xl">
            <span className="text-transparent bg-clip-text bg-gradient-to-b from-white via-white to-indigo-200 drop-shadow-lg">
              Your Path to the
            </span> <br />
            <span className="text-amber-400 drop-shadow-[0_0_15px_rgba(251,191,36,0.5)] animate-text-shimmer">
              Right College
            </span>
          </h1>
          ...
          <button
            ...
            className="group relative px-6 py-3 md:px-8 md:py-4 bg-white text-indigo-700 font-bold rounded-full shadow-2xl hover:shadow-indigo-500/50 transition-all duration-300 hover:-translate-y-1 overflow-hidden text-sm md:text-base"
          >
      ...
      {/* Who We Are - Glassmorphism Card Style */}
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-10 md:gap-16 items-center">
            ...
            <div className="space-y-6 md:space-y-8 text-center lg:text-left">
              <h2
                className={`text-3xl md:text-5xl font-bold text-slate-800 relative inline-block transition-all duration-1000 ease-out ${whoWeAreRef.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                  }`}
              >
                Who We Are
                <div className={`absolute -bottom-2 left-1/2 -translate-x-1/2 lg:left-0 lg:translate-x-0 h-1.5 bg-gradient-to-r from-indigo-500 to-blue-500 rounded-full transition-all duration-1000 delay-500 ${whoWeAreRef.isVisible ? 'w-1/2' : 'w-0'
                  }`}></div>
              </h2>
              <div className="space-y-4 md:space-y-6 text-base md:text-lg text-slate-600 leading-relaxed">
                <p
                  className={`bg-white/60 backdrop-blur-sm p-5 md:p-6 rounded-2xl border border-white shadow-sm hover:shadow-md transition-all duration-1000 delay-300 ease-out ${whoWeAreRef.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                    }`}
                >
                  We are a dedicated team of education counselors based in Dehradun, specializing in admission guidance for private colleges across the city. With years of experience in the education sector, we understand the challenges students and parents face when choosing the right college.
                </p>
                <p
                  className={`bg-white/60 backdrop-blur-sm p-5 md:p-6 rounded-2xl border border-white shadow-sm hover:shadow-md transition-all duration-1000 delay-500 ease-out ${whoWeAreRef.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                    }`}
                >
                  Our mission is simple: to provide <span className="text-indigo-600 font-semibold">honest, transparent, and personalized counseling</span> that helps every student find a college that aligns with their strengths, interests, and future goals.
                </p>
              </div>
            </div>
            ...
            <div
              className={`relative group transition-all duration-1000 delay-700 ease-out ${whoWeAreRef.isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'
                }`}
            >
              ...
              <div className="absolute -bottom-6 -left-6 md:-bottom-10 md:-left-10 z-20 bg-white p-4 md:p-6 rounded-2xl shadow-xl border border-indigo-100 animate-float-delayed hidden md:block">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="font-bold text-slate-800 text-sm md:text-base">Trusted Guidance</div>
                    <div className="text-xs md:text-sm text-slate-500">100% Satisfaction</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {services.map((service, idx) => (
              <div key={idx} className="group p-6 md:p-8 rounded-2xl bg-white border border-slate-200 hover:border-indigo-200 hover:shadow-xl transition-all duration-300 hover:-translate-y-2 relative overflow-hidden text-center">
                ...
                <div className="w-12 h-12 md:w-14 md:h-14 bg-indigo-50 rounded-xl flex items-center justify-center text-indigo-600 mb-4 md:mb-6 group-hover:bg-indigo-600 group-hover:text-white transition-colors duration-300 mx-auto">
                  {service.icon}
                </div>
      ...
      {/* Why Choose Us - Dark Mode Contrast */}
            ...
            <div className={`transform transition-all duration-1000 text-center lg:text-left ${whyChooseRef.isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
              ...
              <div className="space-y-4 md:space-y-6">
                {whyChooseUs.map((item, idx) => (
                  <div
                    key={idx}
                    className={`flex flex-col sm:flex-row gap-4 md:gap-6 p-5 md:p-6 rounded-2xl border border-transparent transition-all duration-500 ease-out hover:bg-white/10 hover:border-white/10 hover:shadow-lg hover:shadow-indigo-500/10 hover:-translate-y-1 group items-center sm:items-start text-center sm:text-left ${whyChooseRef.isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'
                      }`}
                    style={{ transitionDelay: `${idx * 150}ms` }}
                  >
                    <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-indigo-500/20 flex items-center justify-center text-indigo-400 shrink-0 group-hover:scale-110 group-hover:bg-indigo-500 group-hover:text-white transition-all duration-300 shadow-inner">
                      {item.icon}
                    </div>
                    <div>
                      <h3 className="text-lg md:text-xl font-bold mb-1 md:mb-2 text-white group-hover:text-amber-300 transition-colors">{item.title}</h3>
                      <p className="text-slate-200 leading-relaxed font-medium text-sm md:text-base">{item.description}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 text-center divide-x divide-white/20">
            {trustStats.map((stat, idx) => (
              <div key={idx} className="p-2 md:p-4">
                <div className="text-3xl md:text-5xl font-black mb-2 flex justify-center">
                  {trustRef.isVisible && <CountUpAnimation target={stat.number} />}
                </div>
          ...
          <div className="grid md:grid-cols-2 gap-6 md:gap-8 max-w-5xl mx-auto">
            {approachSteps.map((step, idx) => (
              <div key={idx} className="flex gap-4 md:gap-6 items-start bg-white p-6 md:p-8 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-indigo-50">
                <div className="shrink-0 w-12 h-12 md:w-16 md:h-16 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-2xl rotate-3 flex items-center justify-center text-white font-black text-xl md:text-2xl shadow-lg">
                  {step.number}
                </div>
                <div>
                  <h3 className="text-lg md:text-xl font-bold text-slate-900 mb-2 md:mb-3">{step.title}</h3>
                  <p className="text-slate-700 leading-relaxed font-medium text-sm md:text-base">{step.description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section ref={missionVisionRef.ref} className="py-0 relative z-10">
        <div className="grid md:grid-cols-2">
          ...
          <div className={`relative bg-indigo-950 p-8 md:p-16 lg:p-24 flex flex-col justify-center items-center text-center lg:items-start lg:text-left overflow-hidden group transition-all duration-1000 ${missionVisionRef.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <p className="text-blue-50 leading-relaxed text-lg md:text-xl font-medium relative z-10 max-w-lg mx-auto lg:mx-0">
              To empower students with <span className="text-cyan-300 font-bold">expert guidance</span> and <span className="text-amber-300 font-bold">transparent counseling</span>, helping them make informed decisions about their education path.
            </p>
          </div>

          {/* Vision - Light Premium */}
          <div className={`relative bg-gradient-to-br from-white to-indigo-50 p-8 md:p-16 lg:p-24 flex flex-col justify-center items-center text-center lg:items-start lg:text-left border-t md:border-t-0 md:border-l border-indigo-100 overflow-hidden group transition-all duration-1000 delay-200 ${missionVisionRef.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <p className="text-slate-600 leading-relaxed text-lg md:text-xl font-medium relative z-10 max-w-lg mx-auto lg:mx-0">
              To become the <span className="text-indigo-600 font-bold">most trusted name</span> in admission counseling, recognized for our integrity and dedication to <span className="text-purple-600 font-bold">student success</span>.
            </p>
          </div>
      ...
      <section className="py-16 md:py-24 bg-white text-center relative z-10">
        <div className="container mx-auto px-4 max-w-3xl">
          ...
          <button
            onClick={() => navigate('/contact')}
            className="px-8 py-4 md:px-10 md:py-5 bg-indigo-600 text-white font-bold rounded-full shadow-xl hover:bg-indigo-700 hover:scale-105 transition-all duration-300 text-sm md:text-base"
          >
