...
export default function TopCoursesSection({ college }) {
    ...
    const handleFeesClick = () => {
        if (college?.feesStructureLinks && college.feesStructureLinks.length > 0) {
            setShowFeesPopup((prev) => !prev);
            return;
        }
        if (college?.feesStructureImage) {
            window.open(getAssetUrl(college.feesStructureImage), '_blank');
            return;
        }
        if (college?.id) {
            router.push(`/college/${college.id}/fees`);
        }
    };
            ...
            <div className="relative z-10 max-w-6xl mx-auto">
                ...
                <div className={`text-center mt-12 transition-all duration-700 delay-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                        <button
                            onClick={() => router.push('/apply')}
                            className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl bg-emerald-600 text-white font-bold text-sm shadow-xl shadow-emerald-200 hover:bg-emerald-700 hover:shadow-emerald-300 transform hover:-translate-y-0.5 transition-all duration-300 group"
                        >
                            Apply for Admission
                        ...
                        <div className="relative" ref={feesPopupRef}>
                            ...
                            >
                                ...
                                {college?.feesStructureLinks && college.feesStructureLinks.length > 0 ? (
                                    <svg className={`w-4 h-4 transition-transform duration-300 ${showFeesPopup ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
